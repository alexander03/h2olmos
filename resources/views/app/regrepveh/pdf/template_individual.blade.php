<!DOCTYPE html>
<html lang="es">
<head>
    <title>{{ $namefile }}</title>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  
    <style type="text/css">

        @page {
            margin: 0mm;
            border: 0mm;
        }
        body{
            font-size: 8px;
            font-family: "Arial";
        }
        table{
            border-collapse: collapse;
            border: 1.5px;
        }
        tr{
            border-collapse: collapse;
            border: 1.5px solid;
        }
        td{
            border-collapse: collapse;
            border: 1.5px solid;
            height: 20px;
        }
        span{
            font-weight: normal;
        }
        .table-bordered{
            border-collapse: collapse;
            border: 1.5px solid; 
        }
        .table-borderless{
            border-collapse: collapse;
            border: none;
        }
        .table-borderlight{
            border-collapse: collapse;
            border: 0.5px;
        }
        .bg-blue{
            background-color: blue;
        }
        .h2{
            font-size: 12px;
        }
        .text-top{
            vertical-align: text-top;
        }
        .text-center{
            text-align: center;
        }
        .text-white{
            color: white;
        }
        .text-bold{
            font-weight: bold;
        }
        .fsize{
            font-size: 8px;
        }
        .text-normal{
            font-weight: normal;
        }
        #pequeno{
            height: 15px;
        }
    </style>
</head>
<body>
    <table width="100%">
        <tr class="table-borderless">
            <td class="table-borderless">
                
                <table width="100%">
                    <tr>
                        <td rowspan="4" width="15%" style="height: 10px" class="text-center pequeno">
                            <img src="data:image/jpeg;base64," width="100" height="100" />
                        </td>
                        <td rowspan="4" width="55%" style="background:#0070C0;font-size:16"  class="text-center pequeno text-white bg-blue text-bold">
                            REGISTRO DE REPUESTOS VEHICULAR
                            <br>
                            (TALLER CAMPAMENTO)
                        </td>
                        <td>
                            <span class="text-bold pequeno" style="height: 15px">CÓDIGO DEL FORMATO:</span>
                        </td>
                        <td>
                            <span class="text">AD-012</span>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <span class="text-bold">REVISIÓN DEL FORMATO</span>
                        </td>
                        <td>
                            <span class="text">00</span>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <span class="text-bold">FORMATO VIGENTE DESDE:</span>
                        </td>
                        <td>
                            <span class="text">24/JUL/2018</span>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <span class="text-bold">PÁGINA DEL FORMATO:</span>
                        </td>
                        <td>
                            <span class="text">1 DE 1</span>
                        </td>
                    </tr>
                </table>

            </td>
        </tr>
        <tr class="table-borderless">
            <td class="table-borderless">

                <table width="100%">
                    <tr>
                        <td colspan="2">

                            <table width="100%" class="table-borderless">
                                <tr class="table-borderless">
                                    <td width="80%" class="table-borderless text-bold">CONCESIÓN:&nbsp;<span>Operación y Mantenimiento de las Obras de Trasvase del Proyecto Olmos </span></td>
                                </tr>
                            </table>

                        </td>
                        <td  class="text-bold">FECHA DEL REGISTRO</td>
                    </tr>
                    <tr>
                        <td width="45%" class="text-bold">CONCESIONARIA:&nbsp;<span>{{ $concesionaria }}</span></td>
                        <td width="30%" class="text-bold">CLIENTE:&nbsp;<span>{{ $cliente }}</span></td>
                        <td width="25%" class="text-bold"><span></span></td>
                    </tr>
                </table>

            </td>
        </tr>
        <tr class="table-borderless">
            <td class="table-borderless">

                <table width="100%">
                    <tr>
                        <td colspan="4" class="text-center text-bold" style="background: #bdd7ee"><h2>DATOS GENERALES</h2></td>
                    </tr>
                    
                    <tr>
                        <td width="30%" class="text-bold">UNIDAD:&nbsp;<span></span></td>
                        <td width="35%" class="text-bold">MARCA:&nbsp;<span></span></td>
                        <td colspan="2" class="text-bold table-borderless">TIPO DE MANTENIMIENTO:</td>
                    </tr>
                    <tr>
                        <td class="text-bold">PLACA:&nbsp;<span></span></td>
                        <td class="text-bold">MODELO:&nbsp;<span></span></td>
                        <td colspan="2" ROWSPAN="2" class="table-borderless">{{$tipomantenimiento}}</td>
                    </tr>
                    <tr>
                        <td class="text-bold">UA:&nbsp;<span>{{ $ua_id }}</span></td>
                        <td class="text-bold">KM DE MANTENIMIENTO:&nbsp;<span>{{ $kmman }}</span></td>
                    </tr>
                    <tr>
                        <td class="text-bold">KM DE INGRESO:&nbsp;<span>{{ $kminicial }}</span></td>
                        <td class="text-bold">KM DE SALIDA:&nbsp;<span>{{ $kmfinal }}</span></td>
                        <td colspan="2" class="table-borderless">TELEFONO:</td>
                    </tr>
                    <tr>
                        <td class="text-bold">FECHA DE INGRESO:&nbsp;<span>{{ $fechaentrada }}</span></td>
                        <td class="text-bold">FECHA DE SALIDA:&nbsp;<span>{{ $fechasalida }}</span></td>
                        <td colspan="2" class="table-borderless">&nbsp;<span>{{ $telefono }}</span></td>
                    </tr>
                </table>

            </td>
        </tr>
        <tr class="table-borderless">
            <td class="table-borderless">

                <table width="100%">
                    <tr>
                        <td colspan="6" class="text-center text-bold" style="background: #bdd7ee"><h2>OBSERVACIONES</h2></td>
                    </tr>
                    <tr style="background: #bdd7ee">
                        <td width="6%" class="text-bold" style="text-align: center;">ITEM</td>
                        <td width="7%" class="text-bold" style="text-align: center;">CANT.</td>
                        <td width="6%" class="text-bold" style="text-align: center;">UND.</td>
                        <td width="7%" class="text-bold" style="text-align: center;">CODIGO</td>
                        <td width="66%" class="text-bold" style="text-align: center;">DESCRIPCIÓN DE REPUESTOS</td>
                        <td width="8%" class="text-bold" style="text-align: center;">COSTO</td>
                    </tr>
                    <?php $total=0 ?>
                    <?php $contador=0 ?>
                    @foreach($observaciones as $key => $obs)
                        <?php $contador++; ?>
                    <tr>
                        <td width="6%" class="text">{{$contador}}</td>
                        <td width="7%" class="text">{{$obs->cantidad}}</td>
                        <td width="6%" class="text">{{$obs->unidad}}</td>
                        <td width="7%" class="text">{{$obs->codigo}}</td>
                        <td width="66%" class="text">{{$obs->descripcion}}</td>
                        <td width="8%" class="text">{{$obs->monto}}</td>
                        <?php $total+=$obs->monto ?>
                    </tr>
                    @endforeach
                    <tr class="table-borderless">
                        <td width="6%" class="text table-borderless"></td>
                        <td width="7%" class="text table-borderless"></td>
                        <td width="6%" class="text table-borderless"></td>
                        <td width="7%" class="text table-borderless"></td>
                        <td width="66%" class="text table-borderless" align="right">Total</td>
                    <td class="table-bordered" align="right">{{$total}}</td>
                    </tr>
                    <tr class="table-borderless">
                        <td width="6%" class="text table-borderless"></td>
                        <td width="7%" class="text table-borderless"></td>
                        <td width="6%" class="text table-borderless"></td>
                        <td width="7%" class="text table-borderless"></td>
                        <td width="66%" class="text table-borderless" align="right">IGV</td>
                        <td width="8%" class="table-bordered" align="right">10</td>
                    </tr>
                    <tr class="table-borderless">
                        <td width="6%" class="text table-borderless"></td>
                        <td width="7%" class="text table-borderless"></td>
                        <td width="6%" class="text table-borderless"></td>
                        <td width="7%" class="text table-borderless"></td>
                        <td width="66%" class="text table-borderless" align="right">TOTAL</td>
                        <td width="8%" class="table-bordered" align="right">10</td>
                    </tr>
                </table>

            </td>
        </tr>
        <tr></tr>
        <tr class="table-borderless">
            <td class="table-borderless">

                <table width="100%">
                    <tr style="background: #bdd7ee">
                        <td width="33%" class="text-bold" align="center"><h2>CONDUCTOR</h2></td>
                        <td width="33%" class="text-bold" align="center"><h2>AUTORIZA</h2></td>
                        <td width="33%" class="text-bold" align="center"><h2>JEFE DE TALLER</h2></td>
                    </tr>
                    <tr>
                        <td width="33%" class="text-bold"><h2><br><br>FIRMA:</h2></td>
                        <td width="33%" class="text-bold"><h2><br><br>FIRMA:</h2></td>
                        <td width="33%" class="text-bold"><h2><br><br>FIRMA:</h2></td>
                    </tr>
                    <tr>
                        <td width="33%" class="text-bold"><h2>NOMBRE:</h2></td>
                        <td width="33%" class="text-bold"><h2>NOMBRE:</h2></td>
                        <td width="33%" class="text-bold"><h2>NOMBRE:</h2></td>
                    </tr>
                    <tr>
                        <td width="33%" class="text-bold"><h2>TELEFONO</h2></td>
                        <td width="33%" class="text-bold"><h2>TELEFONO</h2></td>
                        <td width="33%" class="text-bold"><h2>TELEFONO</h2></td>
                    </tr>

                </table>

            </td>
        </tr>
        <tr class="table-borderless">
            <td align="center" style="font-size: 7px"><br>“Este documento es propiedad de CONCESIONARIA TRASVASE OLMOS S.A. y no podrá ser copiado o transferido a terceros sin autorización de la empresa”</td>
        </tr>
    </table>
</body>
</html>
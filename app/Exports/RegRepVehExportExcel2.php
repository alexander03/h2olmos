...
class RegRepVehExportExcel2 implements FromCollection, WithHeadings, WithEvents{
    ...
    public function collection(){
        ...
        $consultaGeneral = RegRepVeh::select('repuesto.descripcion as DESCRIPCION','unidad.descripcion as UM','descripcionregrepveh.cantidad as CANTIDAD','descripcionregrepveh.monto AS COSTO','regrepveh.tipomantenimiento AS TIPOMANTENIMIENTO','descripcionregrepveh.monto AS COSTO','ua.codigo AS UA','regrepveh.fecharegistro AS FECHAMANTENIMIENTO','regrepveh.kmman AS KMMANTENIMIENTO','regrepveh.kminicial AS KMINICIAL','regrepveh.kmfinal AS KMFINAL','vehiculo.modelo AS UNIDAD','vehiculo.placa AS PLACA')
            ...
            -> whereNull('regrepveh.deleted_at')-> get();

        $placas= DB::select(`SELECT  distinct vehiculo.placa AS PLACA FROM 'regrepveh' join 'descripcionregrepveh' on regrepveh.id=descripcionregrepveh.regrepveh_id join 'vehiculo' on vehiculo.id=regrepveh.ua_id join 'ua' on ua.id=vehiculo.ua_id join 'repuesto' on repuesto.id=descripcionregrepveh.repuesto_id join 'unidad' on unidad.id=repuesto.unidad_id`)->get();
        $repuestos= DB::select(`SELECT  distinct repuesto.descripcion AS REPUESTO FROM 'regrepveh' join 'descripcionregrepveh' on regrepveh.id=descripcionregrepveh.regrepveh_id join 'vehiculo' on vehiculo.id=regrepveh.ua_id join 'ua' on ua.id=vehiculo.ua_id join 'repuesto' on repuesto.id=descripcionregrepveh.repuesto_id join 'unidad' on unidad.id=repuesto.unidad_id`)->get();
